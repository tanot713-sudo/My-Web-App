...
(function () {
  ...
  /* ⚠️ เพิ่ม 2026-09-18: มีรายงานว่าเจอข้อความ error ดิบเกี่ยวกับ "พื้นที่ไม่พอ" ทั้งที่ไม่ได้เปิดโปรแกรม
     อื่นในเครื่องเลย — คนละกรณีกับ bad_alloc ด้านบน (นั่นคือ "แรม" ไม่พอตอนรันโมเดล) นี่คือ QuotaExceededError
     ของ Cache Storage/IndexedDB ที่เบราว์เซอร์ใช้เก็บไฟล์โมเดลที่ดาวน์โหลดมาแล้ว (จะได้ไม่ต้องโหลดซ้ำทุกครั้ง)
     — เกิดได้แม้ RAM เหลือเยอะ ถ้า "พื้นที่ดิสก์" เครื่องเหลือน้อย หรือเบราว์เซอร์ตั้งเพดาน storage ต่อเว็บไซต์
     ไว้ต่ำ (ไม่เกี่ยวกับโปรแกรม/แท็บอื่นที่เปิดอยู่เลย ตรงตามที่ผู้ใช้รายงาน) แยกข้อความอธิบายออกจากกันให้ชัดว่า
     เป็นคนละปัญหา (ดิสก์ vs แรม) เพื่อให้ผู้ใช้แก้ถูกจุด */
  function friendlyChatError(rawMessage) {
    ...
    if (/QuotaExceededError|quota.{0,20}exceeded|not enough.{0,10}(space|storage)|no space left/i.test(msg)) {
      return 'บันทึกไฟล์โมเดล AI ไม่สำเร็จ เพราะพื้นที่จัดเก็บของเบราว์เซอร์สำหรับเว็บไซต์นี้เต็ม (คนละเรื่อง' +
        'กับโปรแกรม/แท็บอื่นที่เปิดอยู่ — เป็นพื้นที่ดิสก์ที่เบราว์เซอร์กันไว้ให้เว็บนี้โดยเฉพาะ) ลองล้างข้อมูล' +
        'เว็บไซต์นี้ (ตั้งค่าเบราว์เซอร์ > ความเป็นส่วนตัว > ล้างข้อมูลเว็บไซต์ เลือกเฉพาะเว็บนี้) หรือเพิ่มพื้นที่' +
        'ว่างในดิสก์ แล้วลองใหม่อีกครั้ง';
    }
    return msg;
  }
